...
const Skills = () => {
    ...
    <div className="my-5">
      {Object.keys(skillsData).map((category) => {
        ...
        return (
          <div key={category} className="mb-10 px-5 py-2 borderNew2">
            <h2 className="text-2xl font-semibold mb-6">{category}</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3  md:grid-cols-4 lg:grid-cols-6 gap-6">
              {skills.map((skill) => (
                <div
                  key={skill.name}
                  className="flex flex-col items-center p-4 rounded-lg shadow hover:shadow-lg transition-transform transform hover:-translate-y-2"
                >
                  <img
                    src={skill.image}
                    alt={skill.name}
